...
export class EditorPanel {
    ...
    async savePrompt() {
        ...
        if (this.isSaving) {
            logger.debug('[EditorPanel] Save already in progress, ignoring...');
            return;
        }
        ...
        try {
            this.isSaving = true;
            const saveBtn = document.querySelector('#prompt-edit-modal .btn-primary');
            if (saveBtn) {
                saveBtn.disabled = true;
                this.originalSaveBtnText = saveBtn.innerText;
                saveBtn.innerText = i18n.t('saving') || 'Saving...';
            }
            ...
            let savedPrompt;
            logger.debug(`[EditorPanel] Saving prompt. Current ID in DOM: "${id}"`);

            if (id) {
                // Pass command if available to help with recovery
                const commandInput = document.getElementById('prompt-edit-command');
                if (commandInput && commandInput.value) {
                    promptData.command = commandInput.value;
                }
                ...
                // CRITICAL: Update hidden ID field in case of ID transition (legacy/local -> UUID)
                if (savedPrompt && savedPrompt.id && savedPrompt.id !== id) {
                    logger.debug(`[EditorPanel] Updating hidden ID field after transition: ${id} -> ${savedPrompt.id}`);
                ...
                if (savedPrompt && savedPrompt.id) {
                    logger.debug(`[EditorPanel] Created success. Setting hidden ID field to: ${savedPrompt.id}`);
                    document.getElementById('prompt-edit-id').value = savedPrompt.id;

                    // Also update command
                    if (savedPrompt.command) {
                        const cmdInput = document.getElementById('prompt-edit-command');
                        if (cmdInput) cmdInput.value = savedPrompt.command;
                    }

                    // Double check
                    const checkId = document.getElementById('prompt-edit-id').value;
                    logger.debug(`[EditorPanel] Verified hidden ID field value: ${checkId}`);
                } else {
                    logger.warn('[EditorPanel] Created prompt but no ID returned!', savedPrompt);
                }
            }
            ...
            // Update lists - DEBOUNCED to prevent race conditions with DOM updates above
            if (this.panelManager.managementPanel) {
                setTimeout(() => {
                    this.panelManager.managementPanel.updatePromptList();
                    this.panelManager.managementPanel.updateCategoryList();
            ...
            this.panelManager.domUtils.showAlert(`${i18n.t('save_failed') || 'Save failed'}: ${error.message}`);
        } finally {
            this.isSaving = false;
            const saveBtn = document.querySelector('#prompt-edit-modal .btn-primary');
            if (saveBtn) {
                saveBtn.disabled = false;
                saveBtn.innerText = this.originalSaveBtnText || 'Save';
            }
        }
    }
    ...
    openPromptModal(prompt = null) {
        ...
        if (prompt) {
            ...
            const cmdInput = document.getElementById('prompt-edit-command');
            if (cmdInput) cmdInput.value = prompt.command || '';
...
${tableRows}
    ...
    prefillForm(data) {
        ...
        if (commandInput) commandInput.value = data.command || '';
