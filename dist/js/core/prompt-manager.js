...
export class PromptManager {
    constructor() {
        ...
        this.idMap = new Map(); // Track ID transitions (e.g., legacy p-xxx -> official UUID)
    ...
    async loadData(onUpdate) {
        ...
        try {
            logger.debug('🔄 [Background] Starting proactive sync and health check...');

            // 1. Proactively ensure server capabilities (Version Check)
            await this.api.ensureCapabilities();

            // 2. Fetch User and Prompts
            const currentUser = await this.api.getCurrentUser();
            const apiPrompts = await this.api.getAllPrompts();

            this.currentUser = currentUser;

            // 3. Centralized Metadata Health Check (Detection -> Init/Upgrade -> Completion)
            await this.checkAndRestoreMetadata(apiPrompts);

            const freshPrompts = await this.convertAllFromOpenWebUIFormat(apiPrompts);

            // CRITICAL ORDER: 
            // 1. First enhance/fingerprint (this handles rename migration)
            this.enhancePromptsWithMetadata(freshPrompts);

            // 2. Then sync (this cleans up truly orphaned records)
    ...
    async checkAndRestoreMetadata(apiPrompts) {
        logger.debug('📦 Checking and restoring metadata store...');
        let metadata = await this.api.getMetadataStore();

        if (!metadata) {
            logger.debug('📦 Metadata store does not exist, initializing...');
            await this.initializeMetadataStore(apiPrompts); // Pass apiPrompts for initial population
            metadata = await this.api.getMetadataStore(); // Re-fetch after initialization
        }

        this.metadata = metadata;
        await this.ensureMetadataUpToDate(); // Ensure structure is up to date

        // Re-merge categories after metadata is loaded/synced
        if (this.metadata && this.metadata.categories) {
            this.categories = this.mergeCategories(getDefaultCategories(), this.metadata.categories);
        }
        logger.debug('✅ Metadata store checked and restored.');
    }
    ...
    async ensureMetadataUpToDate() {
        ...
        const metadataVersion = this.metadata.version || '1.0';

        if (metadataVersion === currentVersion) {
            return;
        }

        logger.info(`🔄 Upgrading metadata structure: ${metadataVersion} -> ${currentVersion}`);

        let hasChanges = false;

        // Migration: v1.0/1.1 -> v1.2 (Add settings and enhanced prompt fields)
        if (parseFloat(metadataVersion) < 1.2) {
            const defaultMetadata = this.api.getDefaultMetadataStructure();

            // 1. Add missing settings block
            if (!this.metadata.settings) {
                this.metadata.settings = defaultMetadata.settings;
                hasChanges = true;
            }

            // 2. Initialize missing fields for existing prompt entries
            if (this.metadata.prompts) {
                for (const promptId in this.metadata.prompts) {
                    const promptMeta = this.metadata.prompts[promptId];
                    if (promptMeta.description === undefined) {
                        promptMeta.description = '';
                        hasChanges = true;
                    }
                    if (promptMeta.tags === undefined) {
                        promptMeta.tags = [];
                        hasChanges = true;
                    }
                    if (promptMeta.params === undefined) {
                        promptMeta.params = {};
                        hasChanges = true;
                    }
                }
            ...
            this.metadata.version = currentVersion;
            hasChanges = true;
        }

        if (hasChanges) {
            logger.debug('💾 Saving migrated metadata to server...');
            await this.saveMetadataStore();
        }
    }

    // Initialize Metadata Store
    async initializeMetadataStore(existingPrompts = []) {
        logger.debug('🔧 Initializing central metadata store...');

        try {
            // 1. Get existing prompts (if any) - if not passed, try to fetch
            if (!existingPrompts || existingPrompts.length === 0) {
                ...
                logger.debug('📄 Using provided', existingPrompts.length, 'existing business prompts');
            }
            ...
            if (existingPrompts && existingPrompts.length > 0) {
                ...
                for (let i = 0; i < existingPrompts.length; i++) {
                    ...
                    this.metadata.prompts[promptId] = {
                        ...
                        isFavorite: false,
                        description: prompt.title || '', // Initial description from title
    ...
    async syncMetadataWithPrompts(apiPrompts) {
        ...
        for (const prompt of apiPrompts) {
            ...
            if (!this.metadata.prompts[promptId]) {
                ...
                const detectedCategory = this.fallbackCategoryDetection(prompt.content, prompt.title);
                const uuid = (prompt.id && !prompt.id.startsWith('openwebui_')) ? prompt.id : null;

                this.metadata.prompts[promptId] = {
                    uuid: uuid,
                    categoryId: detectedCategory,
                    isFavorite: false,
                    description: '', // New editable field
                    tags: [],        // New business tags
    ...
    enhancePromptsWithMetadata(prompts) {
        ...
        logger.debug('✨ Enhancing prompts with metadata and fingerprinting...');

        let hasMigration = false;

        for (const prompt of prompts) {
            const currentCommand = prompt.command;
            const currentUuid = prompt.id; // In 0.8.0+, this is the UUID

            // 1. Primary Match: Match by exact command key
            let metadataKey = currentCommand;
            let metadata = this.metadata.prompts[metadataKey];

            // 2. Secondary Match (Rename Detection): If no command match, look for UUID fingerprint
            if (!metadata && currentUuid && !currentUuid.startsWith('openwebui_')) {
                // Iterate through metadata to find a matching UUID fingerprint
                for (const [key, meta] of Object.entries(this.metadata.prompts)) {
                    if (meta.uuid === currentUuid) {
                        logger.debug(`🔄 Rename detected! Migrating metadata: "${key}" -> "${currentCommand}" (UUID: ${currentUuid})`);

                        // Migrate metadata to new command key
                        this.metadata.prompts[currentCommand] = { ...meta };
                        delete this.metadata.prompts[key];

                        metadataKey = currentCommand;
                        metadata = this.metadata.prompts[metadataKey];
                        hasMigration = true;
                        break;
                    }
                }
            ...
            if (metadata) {
                // Ensure UUID fingerprint is stored in metadata for future rename detection
                if (!metadata.uuid && currentUuid && !currentUuid.startsWith('openwebui_')) {
                ...
                if (metadata.tags && Array.isArray(metadata.tags)) prompt.tags = metadata.tags;
                if (metadata.params) prompt.params = metadata.params;
            }
        }

        // If any renames were detected and metadata migrated, save the store
        if (hasMigration) {
            this.saveData();
        }
    }
    ...
    async convertAllFromOpenWebUIFormat(openWebUIPrompts) {
        ...
        return openWebUIPrompts.map((p, index) => {
            return {
                id: p.id || p.command?.replace('/', '') || generateUUID(),
                title: p.title || p.name || p.command?.replace('/', '') || 'Untitled Prompt',
                content: p.content || '',
    ...
    async createPrompt(promptData) {
        try {
            ...
            try {
                const result = await this.api.createPrompt(apiPayload);
                // Capture the real UUID from server response immediately
                if (result && (result.id || result.uuid)) {
                    const newId = result.id || result.uuid;
                    logger.debug(`[PromptManager] Created prompt assigned server ID: ${newId}`);
                    if (newId !== promptId) {
                        logger.debug(`[PromptManager] Mapping created in createPrompt: ${promptId} -> ${newId}`);
                        this.idMap.set(promptId, newId);
                    }
                    newPrompt.id = newId;
                }
            } catch (error) {
                ...
                if (error.message.includes('400') || error.message.includes('already registered')) {
                    ...
                    if (result && (result.id || result.uuid)) {
                        const newId = result.id || result.uuid;
                        if (newId !== promptId) {
                            logger.debug(`[PromptManager] Mapping created in createPrompt (suffix): ${promptId} -> ${newId}`);
                            this.idMap.set(promptId, newId);
                        }
                        newPrompt.id = newId;
                    }
                    logger.debug(`✅ Prompt created successfully with suffix: ${commandName}, ID: ${newPrompt.id}`);
                } else {
                    logger.error('[PromptManager] Create failed with 400 and no ID returned');
    ...
    async updatePrompt(promptId, promptData) {
        try {
            // Use getPromptById for robust ID resolution (handles idMap, command fallback, case-sensitivity)
            let oldPrompt = this.getPromptById(promptId);
            const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
            let realId = null;

            // RECOVERY MECHANISM: If prompt not found locally, try to recover via Server API (for 0.8.0+ race conditions)
            if (!oldPrompt && this.api.serverCapability.isV080OrNewer && typeof promptId === 'string' && promptId.startsWith('prompt_')) {
                logger.warn(`[PromptManager] Prompt ${promptId} not found locally during update. Attempting absolute recovery from server...`);
                try {
                    const freshPrompts = await this.api.getAllPrompts();
                    // We don't have the full prompt object yet, so we can't search by command efficiently if we don't know it.
                    // But if this was a "save" operation, maybe we can infer? 
                    // Actually, if we are updating, we SHOULD have the command or at least title in promptData if it's a full update.
                    // If promptData is partial (e.g. category only), we are in trouble if we can't find the old prompt.

                    // However, let's check if the ID map has it now (maybe parallel SWR updated it?)
                    const mappedId = this.idMap.get(promptId);
                    if (mappedId) {
                        oldPrompt = this.prompts.find(p => p.id === mappedId);
                    }

                    // If still not found, and we have promptData.command, try to find by command in fresh list
                    if (!oldPrompt && promptData.command) {
                        const targetCmd = promptData.command.replace(/^\//, '').toLowerCase();
                        const match = freshPrompts.find(p => p.command && p.command.replace(/^\//, '').toLowerCase() === targetCmd);
                        if (match) {
                            oldPrompt = match;
                            // Retroactively map it to prevent future failures
                            if (match.id) {
                                this.idMap.set(promptId, match.id);
                                logger.info(`[PromptManager] Recovered prompt via direct command match: ${promptId} -> ${match.id}`);
                            }
                        }
                    }
                } catch (err) {
                    logger.error('[PromptManager] Recovery failed:', err);
                }
            }

            if (!oldPrompt) {
                logger.error(`[PromptManager] Prompt not found for update. ID: ${promptId}. Available IDs: ${this.prompts.map(p => p.id).slice(0, 5).join(', ')}...`);
                throw new Error('Prompt does not exist');
            }

            // Identify UUID from oldPrompt or newly recovered prompt
            if (oldPrompt.id && !oldPrompt.id.startsWith('openwebui_') && uuidRegex.test(oldPrompt.id)) {
                realId = oldPrompt.id;
            } else if (oldPrompt.openwebuiData && oldPrompt.openwebuiData.id && uuidRegex.test(oldPrompt.openwebuiData.id)) {
                realId = oldPrompt.openwebuiData.id;
            }

            const promptIndex = this.prompts.findIndex(p => p.id === oldPrompt.id);
            // Allow promptIndex to be -1 if we recovered it from server but it's not in local list yet (edge case)
            ...
            if (!realId && this.api.serverCapability.isV080OrNewer) {
                try {
                    ...
                    if (match && match.id && uuidRegex.test(match.id)) {
                        realId = match.id;
                        this.idMap.set(promptId, realId); // Register mapping EARLY to prevent race condition
            ...
            if (result && (result.id || result.uuid)) {
                const newId = result.id || result.uuid;
                if (newId !== promptId) {
                    logger.debug(`[PromptManager] ID transitioned: ${promptId} -> ${newId}`);
                    this.idMap.set(promptId, newId);
                    updatedPrompt.id = newId;
                }
            }

            // 2. Update in-memory prompt list
            // Recalculate index to ensure robustness against list changes during async API calls
            const finalIndex = this.prompts.findIndex(p => p.id === oldPrompt.id || (realId && p.id === realId));
            if (finalIndex !== -1) {
                this.prompts[finalIndex] = updatedPrompt;
            } else {
                logger.warn(`[PromptManager] Prompt disappeared from list during update, re-adding: ${updatedPrompt.title}`);
                this.prompts.push(updatedPrompt);
            }
    ...
    async deletePrompt(promptId) {
        try {
            // Use getPromptById for robust ID resolution
            const prompt = this.getPromptById(promptId);

            if (!prompt) {
                logger.error(`[PromptManager] Prompt not found for deletion. ID: ${promptId}`);
                throw new Error('Prompt does not exist');
            }

            const promptIndex = this.prompts.indexOf(prompt);
            if (promptIndex === -1) {
            ...
            if (!command) {
                command = prompt.title ? this.fallbackGenerateCommand(prompt.title) : '';
            }
            ...
            // If command is still empty, use promptId (cleaned) as command
            if (!command) {
                command = promptId.toString().replace(/^openwebui_/, '');
            }

            // Extract real UUID if available
            const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
            let realId = null;

            if (prompt.id && !prompt.id.startsWith('openwebui_') && uuidRegex.test(prompt.id)) {
                realId = prompt.id;
            } else if (prompt.openwebuiData && prompt.openwebuiData.id && uuidRegex.test(prompt.openwebuiData.id)) {
                realId = prompt.openwebuiData.id;
            }

            // [Critical Fix] UUID recovery for deletion on 0.8.0+
            if (!realId && this.api.serverCapability.isV080OrNewer && command) {
                try {
                    logger.debug(`[PromptManager] Missing UUID for deletion of "${command}" on 0.8.0+, identifying...`);
                    const freshPrompts = await this.api.getAllPrompts();
                    const targetCmd = command.toLowerCase();
                    const match = freshPrompts.find(p => p.command && p.command.replace(/^\//, '').toLowerCase() === targetCmd);
                    if (match && match.id && uuidRegex.test(match.id)) {
                        realId = match.id;
                        this.idMap.set(promptId, realId);
                        logger.debug(`[PromptManager] ✅ Identified UUID for deletion: ${realId}`);
                    }
                } catch (err) {
                    logger.warn('[PromptManager] Failed to recover UUID for deletion:', err);
                }
            }

            // 1. Try to delete from OpenWebUI API first
            try {
                logger.debug(`🗑️ Attempting to delete OpenWebUI prompt, command: ${command}, id: ${realId}`);
    ...
    getPromptById(id) {
        ...
        const effectiveId = this.idMap.get(id) || id;

        // DEBUG: Trace lookup
        // if (id.toString().startsWith('prompt_')) {
        //    logger.debug(`[PromptManager] Lookup: ${id} -> Effective: ${effectiveId}. Map Size: ${this.idMap.size}`);
        // }

        // 2. Try lookup with effective ID
        let prompt = this.prompts.find(p => p.id === effectiveId);
        if (prompt) return prompt;

        // 3. Transitional Fallback: If mapped but not found, try original ID
        // (Scenario: Mapping registered but this.prompts list not yet updated in an async updatePrompt task)
        if (effectiveId !== id) {
            prompt = this.prompts.find(p => p.id === id);
            if (prompt) return prompt;
        }

        // 4. Robust Fallback (Command, Case-Insensitive ID, etc.)
        if (typeof id === 'string') {
            const normalizedId = id.trim().toLowerCase();
            const targetCmd = normalizedId.replace(/^openwebui_/, '').replace(/^\//, '');

            prompt = this.prompts.find(p => {
                // Case-insensitive ID match
                if ((p.id || '').toLowerCase() === normalizedId) return true;

                // Command match (normalized)
                const pCommand = (p.command || '').replace(/^\//, '').toLowerCase();
                return pCommand === targetCmd;
            });

            if (prompt) {
                logger.debug(`[PromptManager] getPromptById matched via fallback: ${id} -> ${prompt.id}`);
                return prompt;
            }
        }

        // 5. Hard Fallback: Last resort check of all IDs
        // (In case of weird ID types or edge cases not caught by normalized check)
        prompt = this.prompts.find(p => p.id == id || p.id === String(id));
        if (prompt) return prompt;

        // 6. Desperate Fallback: If id looks like temp ID but we are missing mapping, 
        // try to find by command if the data might have refreshed
        if (typeof id === 'string' && id.startsWith('prompt_')) {
            logger.debug(`[PromptManager] getPromptById desperate fallback for ${id}`);
            // If we can't find by ID/Mapped ID, we are likely in a race condition where
            // prompts list refreshed but we lost the trace. 
        }
        ...
        try {
            ...
            if (Array.isArray(importData)) {
                ...
                for (const promptData of importData) {
                    const exists = this.getPromptById(promptData.id) ||
                        this.prompts.find(p => p.title === promptData.title);
