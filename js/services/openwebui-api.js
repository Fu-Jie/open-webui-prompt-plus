...
export class OpenWebUIAPI {
    ...
    async getAllPrompts() {
        ...
        return prompts.filter(prompt => prompt.command !== '/_');
    }
    ...
    async updateMetadataStore(metadataData) {
        ...
        const promptData = {
            command: '/_',  // Use underscore as command for system metadata storage
            title: 'System Metadata Store',
            content: JSON.stringify(metadataData, null, 2),
            access_control: null
        };

        const response = await fetch(`${this.apiURL}command/_/update`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
    ...
    async updatePrompt(command, promptData) {
        ...
        // Ensure promptData contains correct command field (must start with /)
        const updateData = {
            ...promptData,
            command: (promptData.command && promptData.command.startsWith('/'))
                ? promptData.command  // Already correct format, keep original
                : `/${command}`  // Missing or incorrect format, set to correct format
        };

        const response = await fetch(`${this.apiURL}command/${command}/update`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`
    ...
    async deletePrompt(command) {
        const token = await this.getAuthToken();
        if (!token) {
            throw new Error('Authentication token not found, please ensure you are logged in');
        }

        const response = await fetch(`${this.apiURL}command/${command}/delete`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${token}`
            }
...
Command Name: `;
    ...
    convertFromOpenWebUIFormat(openWebUIPrompt) {
        return {
            id: 'openwebui_' + (openWebUIPrompt.command?.replace('/', '') || generateUUID()),
            title: openWebUIPrompt.title || openWebUIPrompt.command?.replace('/', '') || 'Untitled Prompt',
